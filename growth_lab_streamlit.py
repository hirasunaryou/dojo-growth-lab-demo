...
optimized_defaults = {"N": 0.80, "F": 0.70, "D": 0.60, "V": 0.75, "R": 0.70}

# Optimized sliders are the only editable line (per spec).
p = {
    "N": st.sidebar.slider("N (Trials)", 0.0, 1.0, optimized_defaults["N"], 0.01),
    "F": st.sidebar.slider("F (Feedback)", 0.0, 1.0, optimized_defaults["F"], 0.01),
    "D": st.sidebar.slider("D (Depth)", 0.0, 1.0, optimized_defaults["D"], 0.01),
    "V": st.sidebar.slider("V (Variety)", 0.0, 1.0, optimized_defaults["V"], 0.01),
    "R": st.sidebar.slider("R (Repetition)", 0.0, 1.0, optimized_defaults["R"], 0.01),
}
...
# -----------------------------
# Run three scenarios: no system, training, optimized (editable)
# -----------------------------
no_result = simulate(
...
with colB:
    ...
    for key in PARAM_KEYS:
        ...
        sim2 = simulate(p2, T, S0, v0, Smax_base, delta_Smax, Umax, u0, enable_frontier, gamma)
